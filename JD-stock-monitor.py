...
#############################
flag = 0  # 记录第几次操作
sys_type = platform.system()  # 判断系统类型
mail_receiver = 'example@163.com,example@qq.com'  # 收件邮箱（多个用,分割）
sc_key = ""
# 商品的url
urls = [
        ...
        for url in urls:
            ...
            try:
                ...
                if (response.text.find('无货') > 0):
                    ...
                    sendMail(name=name, url=skuidUrl, timestamp=time_now)  # 发送邮件进行提醒
                    if sc_key != "":  # Server 酱的推送
                        import json
                        send_wxmsg(_sckey=sc_key, _title=name+"  到货提醒", _context=skuidUrl+"\n"+time_now)
